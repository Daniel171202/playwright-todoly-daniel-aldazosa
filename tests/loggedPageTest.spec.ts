...
var randomTaskName = Math.random().toString(36).substring(10);
...
test("Adicionar un proyecto cambiandole el icono por defecto", async ({
  page,
}) => {
  ...
  expect(
    await loggedPage.checkIfProjectExistsById(idRandomProject)
  ).toBeTruthy();

  await loggedPage.clickOnProjectOptions(idRandomProject);
  await loggedPage.changeProjectIconRandom();
  expect(
    await loggedPage.checkIfProjectIconIsNotDefault(idRandomProject)
  ).toBeTruthy();
...
test("Adicionar una tarea a un proyecto y cambiar su prioridad", async ({
  ...
  expect(
    await loggedPage.checkIfTaskChangedPriority(randomTaskName)
  ).toBeTruthy();
});
